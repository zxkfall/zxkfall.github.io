...
FLAC格式的歌曲有自己的一套标签定义标注，因此并不兼容其他格式的标签，比如ID3，APEv2，虽然官网推荐的读写FLAC的工具可以识别并跳过ID3标签，但并不推荐在FLAC歌曲中写入ID3标签。FLAC格式的歌曲有自己的一套标签定义标注，因此并不兼容其他格式的标签，比如ID3，APEv2，虽然官网推荐的读写FLAC的工具可以识别并跳过ID3标签，但并不推荐在FLAC歌曲中写入ID3标签。
